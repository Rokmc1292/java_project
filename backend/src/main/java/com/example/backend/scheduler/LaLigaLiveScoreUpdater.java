...
public class LaLigaLiveScoreUpdater {
    ...
    public void checkStuckLiveMatchesOnStartup() {
        log.info("🔍 [LaLiga] 서버 시작 - LIVE 상태 경기 점검 시작");
        try {
            List<Match> liveMatches = matchRepository.findByStatus("LIVE");
            List<Match> leagueMatches = liveMatches.stream()
                    .filter(m -> m.getLeague().getLeagueId().equals(7L)).toList();
            if (leagueMatches.isEmpty()) { log.info("✅ [LaLiga] LIVE 상태 경기 없음"); return; }
            log.info("⚠️ [LaLiga] LIVE 상태 경기 {}개 발견", leagueMatches.size());
            LocalDateTime now = LocalDateTime.now();
            List<Match> stuckMatches = leagueMatches.stream()
                    .filter(m -> m.getMatchDate().plusHours(3).isBefore(now)).toList();
            if (stuckMatches.isEmpty()) { log.info("✅ [LaLiga] 모든 LIVE 경기가 정상 범위 내"); return; }
            log.info("🔄 [LaLiga] 과거 LIVE 경기 {}개 발견 - FINISHED로 업데이트", stuckMatches.size());
        ...
        } catch (Exception e) { log.error("❌ [LaLiga] LIVE 경기 점검 실패", e); }
    }
