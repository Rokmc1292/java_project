...
/**
 * 라리가 전체 시즌 일정 크롤러
 * 매일 새벽 3시 20분에 2025-2026 시즌 전체 일정 크롤링
 */
@Component
@RequiredArgsConstructor
@Slf4j
public class LaLigaScheduleCrawler {
    ...
    /**
     * 매일 새벽 3시 20분에 라리가 전체 시즌 일정 크롤링
     * cron: "초 분 시 일 월 요일"
     * EPL(3:00), 분데스리가(3:10)와 시간 차이를 두어 충돌 방지
     */
    ...
    public void scheduledCrawling() {
        ...
        if (currentMonth >= 6 && currentMonth <= 7) {
            log.info("⏭️ [스케줄] 라리가 시즌 오프 기간(6-7월) - 크롤링 건너뛰기");
            return;
        }

        log.info("⏰ [스케줄] 라리가 전체 시즌 일정 크롤링 시작 (매일 새벽 3시 20분)");
        crawlFullSeason();
    }
    ...
    public void crawlFullSeason() {
        log.info("=".repeat(60));
        log.info("라리가 2025-2026 시즌 전체 일정 크롤링 시작");
    ...
    public void saveMatchesToDatabase(List<MatchCrawlDto> matchDtos) {
        ...
        // 라리가 리그 조회 (league_id = 7)
        League laLigaLeague;
        try {
            laLigaLeague = entityManager.getReference(League.class, 7L);
        } catch (Exception e) {
            log.error("❌ 라리가 리그 정보를 찾을 수 없습니다. DB에 리그 데이터가 있는지 확인하세요.", e);
            return;
        }
