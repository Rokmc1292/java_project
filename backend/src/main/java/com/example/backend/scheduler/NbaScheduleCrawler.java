...
public class NbaScheduleCrawler {
    ...
    private MatchCrawlDto extractMatchData(WebElement matchElement, String dateText, int year) {
        try {
            ...
            // 네이버 스포츠 NBA는 첫 번째가 원정팀, 두 번째가 홈팀
            // 첫 번째 팀 (원정팀)
            String awayTeam = teamItems.get(0).findElement(By.cssSelector(".MatchBoxHeadToHeadArea_team__l2ZxP")).getText();
            // 두 번째 팀 (홈팀)
            String homeTeam = teamItems.get(1).findElement(By.cssSelector(".MatchBoxHeadToHeadArea_team__l2ZxP")).getText();
    ...
    public void saveMatchesToDatabase(List<MatchCrawlDto> matchDtos) {
        ...
        League nbaLeague;
        try {
            nbaLeague = entityManager.getReference(League.class, 2L);
        } catch (Exception e) {
            log.error("❌ NBA 리그 정보를 찾을 수 없습니다. DB에 리그 데이터가 있는지 확인하세요.", e);
            return;
        }
