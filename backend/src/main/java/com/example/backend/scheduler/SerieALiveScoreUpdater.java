...
public class SerieALiveScoreUpdater {
    ...
    public void checkStuckLiveMatchesOnStartup() {
        log.info("🔍 [SerieA] 서버 시작 - LIVE 상태 경기 점검 시작");
        try {
            List<Match> liveMatches = matchRepository.findByStatus("LIVE");
            List<Match> leagueMatches = liveMatches.stream()
                    .filter(m -> m.getLeague().getLeagueId().equals(8L)).toList();
            if (leagueMatches.isEmpty()) { log.info("✅ [SerieA] LIVE 상태 경기 없음"); return; }
            log.info("⚠️ [SerieA] LIVE 상태 경기 {}개 발견", leagueMatches.size());
            LocalDateTime now = LocalDateTime.now();
            List<Match> stuckMatches = leagueMatches.stream()
                    .filter(m -> m.getMatchDate().plusHours(3).isBefore(now)).toList();
            if (stuckMatches.isEmpty()) { log.info("✅ [SerieA] 모든 LIVE 경기가 정상 범위 내"); return; }
            log.info("🔄 [SerieA] 과거 LIVE 경기 {}개 발견 - FINISHED로 업데이트", stuckMatches.size());
        ...
        } catch (Exception e) { log.error("❌ [SerieA] LIVE 경기 점검 실패", e); }
    }
