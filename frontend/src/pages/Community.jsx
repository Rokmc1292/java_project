...
function Community() {
    ...
    const [categories, setCategories] = useState(['전체']);  // ⭐ 동적으로 로드
    const [writableCategories, setWritableCategories] = useState([]);  // ⭐ 동적으로 로드
    ...
    useEffect(() => {
        const loadCategories = async () => {
            try {
                ...
                // 전체 카테고리 목록 (전체 + DB 카테고리)
                setCategories(['전체', ...categoryNames]);

                // 글쓰기 가능한 카테고리 목록 (DB 카테고리만)
                ...
                // 실패 시 기본값 사용
                const defaultCategories = ['축구', '야구', '농구', '롤', 'UFC', '자유게시판'];
                setCategories(['전체', ...defaultCategories]);
    ...
    const handleCreatePost = async () => {
        ...
        try {
            console.log('작성 요청:', newPost);  // ⭐ 디버깅용
